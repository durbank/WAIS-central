# This script is for comparing manually-selected smb results vs PAIPR-generated SMB results, as well as comparing repeatability between 2011 and 2016, both with manual results and with PAIPR results
...
gdf_traces2011 = gpd.GeoDataFrame(
    {'ID_man': dist_overlap1.index, 
    'ID_PAIPR': dist_overlap1['trace_ID'], 
    'accum_man': man2011_accum.mean(axis=0), 
    'accum_PAIPR': Maccum_2011.mean(axis=0)},
    geometry=dist_overlap1.geometry.values)


# Calculate residuals (as % bias of mean accumulation)
res_2011 = (
    (Maccum_2011 - man2011_accum) 
    / np.mean(
        [Maccum_2011.mean(axis=0), 
        man2011_accum.mean(axis=0)], axis=0))
...
print(
    ...
    f"{(man2011_std.mean()/man2011_accum.mean())*100:.2f}% " 
    f"for 2011 manual layers and "
    f"{(Mstd_2011.mean()/Maccum_2011.mean())*100:.2f}% "
...
# %%
idx_i = PIG_idx
df_paipr = pd.DataFrame(
    Maccum_2011[:,idx_i], 
    index=a2011_ALL.index)
df_manual = pd.DataFrame(
    man2011_accum[:,idx_i], 
    index=man2011_ALL.index)

df_paipr.mean(axis=1).plot(
    color='red', linewidth=2, 
    label='2011 PAIPR')
(
    df_paipr.mean(axis=1) 
    + df_paipr.std(axis=1)).plot(
        color='red', linestyle='--', 
        label='__nolegend__')
(
    df_paipr.mean(axis=1) 
...
df_manual.mean(axis=1).plot(
    ...
    label='2011 manual')
(
    df_manual.mean(axis=1) 
    + df_manual.std(axis=1)).plot(
        color='blue', linestyle='--', 
        label='__nolegend__')
(
    df_manual.mean(axis=1) 
...
df_2011 = pd.DataFrame(
    ...
    'accum_man': np.reshape(
        man2011_accum, man2011_accum.size),  
    'accum_paipr': np.reshape(
        Maccum_2011, Maccum_2011.size)})
...
trend_man, intcpt_man, lb_man, ub_man = trend_bs(
    pd.DataFrame(
        man2011_accum, index=man2011_ALL.index), 
    1000, 
    df_err=pd.DataFrame(
        man2011_std, index=man2011_ALL.index))

trend_paipr,intcpt_paipr,lb_paipr,ub_paipr = trend_bs(
    pd.DataFrame(
        Maccum_2011, index=man2011_ALL.index), 
    1000, 
    df_err=pd.DataFrame(
        Mstd_2011, index=man2011_ALL.index))
...
man2016_accum = man2016_ALL.iloc[
    :,dist_overlap2.index].to_numpy()
man2016_std = manSTD_2016_ALL.iloc[
    :,dist_overlap2.index].to_numpy()
Maccum_2016 = a2016_ALL.iloc[
    :,dist_overlap2['trace_ID']].to_numpy()
Mstd_2016 = std2016_ALL.iloc[
    :,dist_overlap2['trace_ID']].to_numpy()

# Create new gdf of subsetted results
gdf_traces2016 = gpd.GeoDataFrame(
    {'ID_man': dist_overlap2.index, 
    'ID_PAIPR': dist_overlap2['trace_ID'], 
    'accum_man': man2016_accum.mean(axis=0), 
    'accum_PAIPR': Maccum_2016.mean(axis=0)},
    geometry=dist_overlap2.geometry.values)


# Calculate residuals (as % bias of mean accumulation)
res_2016 = (
    (Maccum_2016 - man2016_accum) 
    / np.mean(
...
print(
    f"The mean standard deviations of the annual "
    f"accumulation estimates are "
    f"{(man2016_std.mean()/man2016_accum.mean())*100:.2f}% " 
    f"for 2016 manual layers and "
    f"{(Mstd_2016.mean()/Maccum_2016.mean())*100:.2f}% "
...
res_PAIPR = (
    ...
    / np.mean(
        [accum_2016.mean(axis=0), 
        accum_2011.mean(axis=0)], axis=0))
...
print(
    f"The mean standard deviations of the annual "
    f"accumulation estimates are "
    f"{(std_2011.mean()/accum_2011.mean())*100:.2f}% " 
    f"for the 2011 flight and "
    f"{(std_2016.mean()/accum_2016.mean())*100:.2f}% "
    f"for the 2016 flight."
)
# %%
trend_2011, intcpt, lb_2011, ub_2011 = trend_bs(
    pd.DataFrame(
        accum_2011, index=a2011_ALL.index), 
    1000, 
    df_err=pd.DataFrame(
        std_2011, index=std2011_ALL.index))

trend_2016, intcpt, lb_2016, ub_2016 = trend_bs(
    pd.DataFrame(
        accum_2016, index=a2016_ALL.index), 
    1000, 
    df_err=pd.DataFrame(
        std_2016, index=std2016_ALL.index))
