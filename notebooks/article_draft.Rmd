...
accum.long = data %>% select(Lat, Lon, elev, Year) %>% 
...
cores.accum = left_join(core_long, core.locs, by = "Site")  %>% arrange(Site, Year) %>% 
  select(Site, Latitude, Longitude, Elevation, Year, Accum) %>% 
...
accum.loc_data = select(accum.loc_PS, -data, -accum_mu) %>% 
  mutate(data = map(accum.lm_loc, 
                    ~ select(augment(.), Year, accum, .fitted, .se.fit, .resid, .std.resid)))
...
trend.loc = accum.loc_PS %>% 
  select(Easting, Northing, elev, accum_mu) %>% 
...
# Extract relevant model data and store in nested tibble
cores.loc_data = select(cores.loc_PS, -Data, -accum_mu, -data.length) %>% 
...
cores.trend = cores.loc_PS %>% 
  select(Site, Easting, Northing, Elevation, accum_mu) %>% 
  mutate(intrcpt = simplify(map(tmp.tidy, "estimate") %>% map(1))) %>%
  mutate(coeff_yr = simplify(map(tmp.tidy, "estimate") %>% map(2))) %>% 
  mutate(std.err = simplify(map(tmp.tidy, "std.error") %>% map(2))) %>% 
  mutate(p.val_yr = simplify(map(tmp.tidy, "p.value") %>% map(2)))
...
test.data = select(test.smp, -data, -accum_mu) %>% 
...
index.file = "/media/durbank/WARP/Research/Antarctica/Data/DEMs/REMA/REMA_Tile_Index_Rel1.1/REMA_Tile_Index_Rel1.1.shp"
...
# output.dir = file.path("/media/durbank/WARP/Research/Antarctica/Data/DEMs/REMA/WAIS_divide")
output.dir = tk_choose.dir(caption = "Select directory in which to save REMA mosaic tiles")
...
for (file in 1:nrow(mosaic.download)) {
  ...
  # dir.create(file.path(output.dir, dir.name))
  
  # If not already downloaded, download tile and extract
  if (!dir.exists(file.path(output.dir, dir.name))) {
    download.file(mosaic.download$fileurl[file], tmp.file)
    untar(tmp.file, exdir = file.path(output.dir, dir.name))
  }
  unlink(tmp.file)
}
```
...
# tic("Raster analysis")
# 
# dem.file = "/media/durbank/WARP/Research/Antarctica/Data/DEMs/REMA/WAIS_divide/r.slope.tif"
# dem.slope = raster(dem.file)
# accum.loc_PS = accum.loc_PS %>% mutate(slope = extract(dem.slope, accum.PS))
# 
# toc()
```
